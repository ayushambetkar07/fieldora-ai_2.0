...
export const SUPPORTED_UNITS = ['kg', 'quintal', 'ton', 'tonne', 'crate', 'piece'] as const;
export type SupportedUnit = typeof SUPPORTED_UNITS[number] | string;

// Conversion factors to kilograms
export const UNIT_TO_KG_MAP: Record<string, number> = {
  kg: 1,
  kilogram: 1,
  quintal: 100,
  qtl: 100,
 ...
 */
export function toKg(quantity: number, unit: SupportedUnit = 'kg'): number {
...
export function calculateQuantityScore(
  rfqQty: number,
  rfqUnit: SupportedUnit,
  listingQty: number,
  listingUnit: SupportedUnit
): number {
  const rfqInKg = toKg(rfqQty, rfqUnit);
  const listingInKg = toKg(listingQty, listingUnit);

  if (rfqInKg <= 0) return 0;
  if (listingInKg <= 0) return 0;
