...
export type OfferRole = 'buyer' | 'farmer';
export type OfferStatus = 'active' | 'superseded' | 'accepted' | 'rejected';
...
export interface AcceptRejectInput {
  purchase_request_id: string;
  user_id: string;
  role?: OfferRole;
}
...
export async function createPurchaseRequest(input: CreatePurchaseRequestInput) {
  const {
    ...
    buyer_company = 'Fieldora Verified Procurement',
    requested_quantity,
    unit = 'kg',
    offered_price_per_unit,
    delivery_location = 'Regional Delivery Hub',
    required_date,
    message,
    expires_at
  } = input;
 ...
 */
export async function submitCounterOffer(input: CounterOfferInput) {
  ...
  let effectiveRole: OfferRole = role;
  if (!effectiveRole) {
 ...
 */
export async function acceptDeal(input: AcceptRejectInput) {
  ...
  if (listingId) {
    ...
    const updateListingPayload: any = {
 ...
 */
export async function rejectDeal(input: AcceptRejectInput) {
 ...
 */
export async function cancelPurchaseRequest(purchase_request_id: string, user_id: string) {
  const { data: request, error: reqError } = await supabase
    .from('purchase_requests')
    .select('*')
    .eq('id', purchase_request_id)
 ...
 */
export async function getNegotiationOffers(purchase_request_id: string) {
