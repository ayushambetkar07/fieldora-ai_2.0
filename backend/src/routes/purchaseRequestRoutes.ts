...
// =========================================================================
router.get('/:id/offers', requireAuth, async (req: AuthRequest, res: Response) => {
  try {
    const id = req.params.id as string;
    const offers = await getNegotiationOffers(id);
...
router.post('/:id/counter', requireAuth, async (req: AuthRequest, res: Response) => {
  try {
    ...
    const role = (req.body.role || req.body.offered_by_role || 'farmer') as OfferRole;
...
router.post('/:id/accept', requireAuth, async (req: AuthRequest, res: Response) => {
  try {
    const id = req.params.id as string;
    const user_id = req.user?.id || '039b5a52-cfab-42df-9cbc-22214e210de5';
    const role = req.body.role as OfferRole;
