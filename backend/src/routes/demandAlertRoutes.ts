...
router.get('/intelligence', async (req: Request, res: Response) => {
  try {
    ...
    if (pErr) console.warn('market_prices query note:', pErr.message);
    if (lErr) console.warn('produce_listings query note:', lErr.message);
    if (rErr) console.warn('buyer_requirements query note:', rErr.message);

    const pList = prices || [];
    const lList = listings || [];
    const rList = reqs || [];

    function buildCropDemand(cropKey: string, defaultIcon: string, displayName: string) {
