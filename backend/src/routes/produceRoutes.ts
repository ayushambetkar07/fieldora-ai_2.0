...
router.get('/farmer/:farmerId', requireAuth, async (req: AuthRequest, res: Response) => {
  try {
...
router.patch('/:id', requireAuth, async (req: AuthRequest, res: Response) => {
  try {
    ...
    const updates: any = {};
    if (quantity !== undefined && !isNaN(Number(quantity)) && Number(quantity) > 0) updates.quantity = Number(quantity);
    if (price_per_unit !== undefined && !isNaN(Number(price_per_unit)) && Number(price_per_unit) >= 0) updates.expected_price = Number(price_per_unit);
    if (description !== undefined) updates.description = description;
    if (location && typeof location === 'string') updates.location = location.trim();
    if (quality_grade !== undefined) updates.quality = quality_grade;
    if (status && ['Active', 'cancelled'].includes(status)) updates.status = status;
