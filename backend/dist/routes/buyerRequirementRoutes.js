...
const router = Router();
const VALID_STATUSES = ['open', 'closed', 'cancelled', 'fulfilled'];
...
router.patch('/:id', requireAuth, async (req, res) => {
    try {
        ...
        if (req.body.variety !== undefined)
            updates.variety = req.body.variety ? req.body.variety.trim() : null;
        ...
        if (req.body.target_price !== undefined) {
            ...
            if (isNaN(p) || p < 0)
                return res.status(400).json({ success: false, message: 'target_price must be >= 0' });
            updates.target_price = p;
        }
        if (req.body.delivery_location)
            updates.delivery_location = req.body.delivery_location.trim();
        if (req.body.required_by_date !== undefined)
            updates.required_by_date = req.body.required_by_date;
