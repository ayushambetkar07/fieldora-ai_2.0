...
export const memoryTrustScores = new Map();
export const memoryOrdersCache = new Map();
 ...
 */
function validateRatingBounds(val, fieldName) {
    const num = Number(val);
    if (isNaN(num) || num < 1 || num > 5) {
        const err = new Error(`${fieldName} must be a number between 1 and 5`);
        err.status = 400;
        throw err;
    }
 ...
 */
export function calculateOverallRating(communication, quality, reliability, timeliness) {
 ...
 */
export async function createVerifiedReview(input, authenticatedUserId) {
    if (!authenticatedUserId) {
        const err = new Error('Authentication required to submit a review');
        err.status = 401;
        throw err;
    }
    ...
    if (!order) {
        const err = new Error(`Order #${input.order_id} does not exist`);
        err.status = 404;
        throw err;
    }
    ...
    if (!isDeliveredOrCompleted) {
        const err = new Error(`Order status is '${order.status}' (${order.payment_status}). Reviews are only permitted for delivered or completed trades.`);
        err.status = 400;
        throw err;
    }
    ...
    if (!isBuyer && !isFarmer) {
        const err = new Error('Forbidden: You are not a participant in this order');
        err.status = 403;
        throw err;
    }
    const reviewer_role = isBuyer ? 'buyer' : 'farmer';
    ...
    if (reviewer_id === reviewee_id) {
        const err = new Error('Self-review is strictly prohibited');
        err.status = 400;
        throw err;
    }
    ...
    const overall_rating = calculateOverallRating(input.communication_rating, input.quality_rating, input.reliability_rating, input.timeliness_rating);
 ...
 */
export async function updateReview(reviewId, input, authenticatedUserId) {
    if (!authenticatedUserId) {
        const err = new Error('Authentication required to edit a review');
        err.status = 401;
        throw err;
    }
    let review = null;
    ...
    if (input.comment !== undefined)
        review.comment = String(input.comment).trim();
 ...
 */
export async function deleteReview(reviewId, authenticatedUserId) {
    if (!authenticatedUserId) {
        const err = new Error('Authentication required to delete a review');
        err.status = 401;
        throw err;
    }
    let review = null;
...
export async function recalculateTrustScore(userId) {
    ...
    const memoryList = Array.from(memoryReviews.values()).filter(r => String(r.reviewee_id) === String(userId) && r.verified_trade === true);
    ...
    const round2 = (val) => Math.round((val / count) * 100) / 100;
...
export async function getReviewsForUser(userId) {
    ...
    const memoryList = Array.from(memoryReviews.values()).filter(r => String(r.reviewee_id) === String(userId));
...
export async function getReviewsForOrder(orderId) {
    ...
    const memoryList = Array.from(memoryReviews.values()).filter(r => String(r.order_id) === String(orderId));
    const seenIds = new Set(reviews.map(r => r.id));
    for (const m of memoryList) {
        if (!seenIds.has(m.id)) {
            reviews.push(m);
