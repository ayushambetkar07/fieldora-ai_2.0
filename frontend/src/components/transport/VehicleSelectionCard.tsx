...
export const VehicleSelectionCard: React.FC<VehicleSelectionCardProps> = ({
  ...
  const capacityKg = vehicle.capacityKg || vehicle.capacity || 1000;
  const isOverCapacity = orderWeightKg > capacityKg || vehicle.isSuitable === false;
  const capacityUsagePercent = loadingPercentage !== undefined 
    ? loadingPercentage 
    : Math.min(100, Math.round((orderWeightKg / capacityKg) * 100));

  const isBusy = vehicle.availabilityStatus === 'Busy' || vehicle.availability === 'Busy';
  const displayCost = vehicle.estimatedFreight !== undefined ? vehicle.estimatedFreight : calculatedCost;
  const bestMatch = isRecommended || vehicle.isBestMatch;
  const vehicleName = vehicle.vehicleName || vehicle.name;
  const driverName = vehicle.driverName || 'Verified Driver';
  const vehicleNumber = vehicle.vehicleNumber || 'MH-15-EG-4412';
  const rating = vehicle.rating || 4.8;
  const features = vehicle.features || ['GPS Tracked', 'Express Corridor Fast-Tag'];
    ...
    >
      ...
      <div className="flex items-center justify-between gap-2 mb-3">
        <div className="flex items-center gap-1.5 flex-wrap">
          ...
          {matchScore !== undefined && matchScore > 0 && !bestMatch && (
            <span className="px-2 py-0.5 rounded-full text-[10px] font-bold bg-[#F0FDF4] text-[#166534] border border-[#bbf7d0] flex items-center gap-1">
      ...
      <div className="flex items-start justify-between gap-3">
        <div className="space-y-0.5">
          ...
          <p className="text-xs text-secondary">
            Payload Capacity: <strong className="text-main font-mono">{capacityKg.toLocaleString('en-IN')} kg</strong>
          </p>
        </div>
        ...
        <div className="text-right">
          ...
          <span className="text-[10px] text-muted block">
            {vehicle.perKmRate ? `(₹${vehicle.perKmRate}/km + ₹${vehicle.baseCost || 300} base)` : `Direct Highway Freight`}
          </span>
        </div>
      </div>
      ...
      <div className="mt-4 space-y-1.5 bg-[#F7F9F6] p-2.5 rounded-input border border-border">
        <div className="flex items-center justify-between text-xs">
          ...
          <span className={cn("font-bold font-mono text-[11px]", isOverCapacity ? "text-error" : "text-primary")}>
            {isOverCapacity ? `Exceeds by ${(orderWeightKg - capacityKg).toLocaleString('en-IN')} kg` : `${capacityUsagePercent}% Loaded`}
          </span>
        </div>
      ...
      <div className="mt-4 pt-3 border-t border-border/80 flex items-center justify-between gap-2 text-xs">
          ...
          <div>
            <div className="font-bold text-main leading-tight text-xs flex items-center gap-1">
              ...
              <span className="text-[10px] text-amber-600 font-semibold flex items-center">
                <Star className="w-3 h-3 fill-amber-400 text-amber-400 inline" /> {rating}
              </span>
            </div>
        ...
        {isOverCapacity ? (
          ...
          <span className="text-[11px] font-bold text-amber-700 flex items-center gap-1 bg-amber-50 px-2 py-0.5 rounded-full border border-amber-200">
            <AlertTriangle className="w-3 h-3" /> Busy on Route
          </span>
        ) : (
      ...
      <div className="mt-3 flex items-center gap-1.5 flex-wrap">
        {(vehicle.features || features).map((feat: string, idx: number) => (
          <span 
            key={idx} 
